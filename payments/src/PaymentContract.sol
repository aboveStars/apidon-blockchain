...
contract PaymentContract {
    ...
    function createProviderPaymentRule(
    ...
    function updateProviderPaymentRule(
        uint256 _ID,
        uint256 _newAmount,
        uint16 year,
        uint8 month,
        ...
        require(
            block.timestamp <= rule.dueDate,
            "Due date has passed, cannot update"
        );
    ...
    function getProviderPaymentRuleByID(
        uint256 _ID
    )
        external
        view
    ...
    function makeProviderPayment(uint256 _ID) external payable {
        ...
        require(
            rule.amount > 0,
            "Payment rule with the given ID does not exist"
        );
        require(msg.value == rule.amount, "Incorrect amount sent");
        require(
            rule.recipient == payable(msg.sender),
            "Sender is not the recipient of the payment rule"
        );
    ...
    function getUserPaymentRuleByID(
        uint256 _ID
    )
        external
        view
        onlyOwnerOrAdmin
        returns (address userAddress, uint256 amount, uint256 ID)
    {
    ...
    function getUserPaymentRuleStatus(
        uint256 _ID
    ) external view onlyOwnerOrAdmin returns (bool) {
        UserPaymentRule storage rule = userPaymentRules[_ID];
        return rule.status;
    }

    // Function to get the status of a provider payment rule by its ID, accessible by contract owner and admin
    function getProviderPaymentRuleStatus(
        uint256 _ID
    ) external view onlyOwnerOrAdmin returns (bool) {
        ProviderPaymentRule storage rule = providerPaymentRules[_ID];
        return rule.status;
